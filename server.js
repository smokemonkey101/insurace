...
function getDefaultSettings() {
  return {
    ...
    range: process.env.GOOGLE_SHEETS_RANGE || "Medications!A:M",
...
async function appendRowsToGoogleSheet({ spreadsheetId, range, rows }) {
...
async function uploadFileToGoogleDrive({ fileBuffer, fileName, mimeType, folderId }) {
  const accessToken = await getGoogleAccessToken([
...
async function handleSheetsAppend(req, res) {
  try {
    ...
    const spreadsheetId = String(
      body.spreadsheetId || appSettings.spreadsheetId || ""
    ).trim();
